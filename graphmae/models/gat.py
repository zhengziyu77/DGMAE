...
class GAT(nn.Module):
    ...
    def forward(self, g, inputs, return_hidden=False):
        ...
        for l in range(self.num_layers):
            if l < self.num_layers-1:        
                h,attn = self.gat_layers[l](g, h,True)
            else:
                h,attn = self.gat_layers[l](g, h,True)
        ...
        # output projection
        if return_hidden:
            return self.head(h), hidden_list,attn
        else:
...
class GATConv(nn.Module):
    ...
    def forward(self, graph, feat, get_attention=False):
        with graph.local_scope():
            ...
            #graph.edata['a'] = self.attn_drop(edge_softmax(graph, e, norm_by='src'))
            # message passing
            ...
            if get_attention:
                ...
                #return rst,self.attn_drop(edge_softmax(graph, e, norm_by='src'))
            else:
                return rst
